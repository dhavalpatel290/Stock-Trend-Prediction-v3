...
for onecsvfile in csvfiles:
    ...
    for year in years:
        ...
        tp.append(p)
        tn.append(n)
        pper.append(round((p/tl)*100,3))
        nper.append(round((n/tl)*100,3))
        ...
        if (p/forwhatsplit).is_integer():
            intrp=p/forwhatsplit
            intep=p/forwhatsplit
            trp.append(intrp)
            tep.append(intep)
        else:
        ...
        if (n/forwhatsplit).is_integer():
            intrn=n/forwhatsplit
            inten=n/forwhatsplit
            trn.append(intrn)
            ten.append(inten)
        else:
            if(putSplitMode==1): # --- > to get 10 % training and 10% testing dataset
